...
class MainWindow:
    def __init__(self):
        ...
        try:
            ...
            log_error("Помилка ініціалізації Playwright", e)
            messagebox.showerror("Помилка", "Не вдалося ініціалізувати браузер!")
    ...
    def create_context_menu(self, entry):
        """Створює контекстне меню для поля введення з опцією 'Вставити'."""
    ...
    def on_closing(self):
        ...
        if self.page:
            try:
                ...
                log_error("Помилка закриття сторінки Playwright", e)
        if self.browser:
            try:
                self.browser.close()
            except Exception as e:
                log_error("Помилка закриття браузера Playwright", e)
        if self.playwright:
            try:
                self.playwright.stop()
            except Exception as e:
                log_error("Помилка зупинки Playwright", e)
        self.root.destroy()
    ...
    def setup_record_tab(self):
        ...
        self.templates_canvas = tk.Canvas(template_frame, height=50)
        self.templates_scrollbar = ttk.Scrollbar(template_frame, orient=tk.HORIZONTAL, command=self.templates_canvas.xview)
        ...
        self.templates_canvas.pack(side=tk.TOP, fill=tk.X, padx=1)
        self.templates_scrollbar.pack(side=tk.BOTTOM, fill=tk.X)
        ...
        self.create_context_menu(self.action_name_entry)  # Додаємо контекстне меню
        ...
        self.create_context_menu(self.x_entry)  # Додаємо контекстне меню
        ...
        self.create_context_menu(self.y_entry)  # Додаємо контекстне меню
        ...
        self.create_context_menu(self.extra_entry)  # Додаємо контекстне меню
        ...
        self.create_context_menu(self.delay_entry)  # Додаємо контекстне меню
        ...
        self.actions_list = tk.Listbox(right_frame, height=10, font=DEFAULT_CONFIG["default_font"], width=25)
    ...
    def toggle_extra_input(self, *args):
        ...
        if self.action_type_var.get() in ["введення_тексту", "натискання_клавіші", "пошук_за_xpath", "пошук_за_css"]:
    ...
    def bind_profiles_to_template(self):
        ...
        def save_binding():
            ...
            logger.info(f"Прив’язані профілі до {template_names}: {selected_profiles}")
            bind_window.destroy()
    ...
    def setup_run_tab(self):
        ...
        self.run_templates_scrollable_frame.bind("<Configure>",
                                                lambda e: self.run_templates_canvas.configure(
                                                    scrollregion=self.run_templates_canvas.bbox("all")))
    ...
    def start_recording(self):
        ...
        extra_value = self.extra_entry.get() if action_type in ["введення_тексту", "натискання_клавіші", "пошук_за_xpath", "пошук_за_css"] else ""
        ...
        def monitor_recording():
            ...
            if keyboard.is_pressed('esc'):
                self.recording = False
                self.timer_label.config(text="")
                self.status_label.config(text="Запис скасовано")
                self.record_button.config(state=tk.NORMAL)
                self.delete_action_button.config(state=tk.NORMAL)
                self.edit_action_button.config(state=tk.NORMAL)
                self.cancel_edit_button.config(state=tk.NORMAL if self.edit_mode else tk.DISABLED)
                self.move_up_button.config(state=tk.NORMAL)
                self.move_down_button.config(state=tk.NORMAL)
                logger.info("Запис скасовано через Esc")
            else:
    ...
    def fix_action(self):
        ...
        if action_type in ["введення_тексту", "натискання_клавіші", "пошук_за_xpath", "пошук_за_css"]:
            ...
            elif action_type in ["пошук_за_xpath", "пошук_за_css"]:
                action["selector"] = extra_value
        ...
        for template_name in selected_templates:
            if template_name in self.action_manager.data["templates"]:
                for idx, action in enumerate(self.action_manager.data["templates"][template_name]["actions"], 1):
                    ...
                    elif action["type"] in ["пошук_за_xpath", "пошук_за_css"]:
                        extra_info = f" ({action.get('selector', '')})"
    ...
    def delete_action(self):
        ...
        for template_name in selected_templates:
            action = self.action_manager.data["templates"][template_name]["actions"][action_index]
            self.undo_stack.append(("delete", template_name, action_index, action))
            del self.action_manager.data["templates"][template_name]["actions"][action_index]
        self.action_manager.save_actions()
        self.update_actions_list()
    ...
    def edit_action_in_window(self):
        ...
        self.create_context_menu(action_name_entry)  # Додаємо контекстне меню
        ...
        self.create_context_menu(delay_entry)  # Додаємо контекстне меню
        ...
        self.create_context_menu(extra_entry)  # Додаємо контекстне меню
        Tooltip(extra_entry, TEXTS["tooltip_extra_input"])
        if action["type"] in ["введення_тексту", "натискання_клавіші", "пошук_за_xpath", "пошук_за_css"]:
            extra_value = action.get("text", action.get("key", action.get("selector", "")))
        ...
        def toggle_extra_input(*args):
            ...
            if current_type in ["введення_тексту", "натискання_клавіші", "пошук_за_xpath", "пошук_за_css"]:
                extra_frame.pack(fill=tk.X, padx=5, pady=2)
                extra_entry.delete(0, tk.END)
                if current_type == action["type"]:
                    extra_value = action.get("text", action.get("key", action.get("selector", "")))
                    extra_entry.insert(0, extra_value)
                    logger.info(f"Відновлено значення Текст/Клавіша/Селектор: {extra_value}")
                else:
                    logger.info("Поле Текст/Клавіша/Селектор очищено для нового типу")
        ...
        def save_edit():
            ...
            elif new_action_type in ["пошук_за_xpath", "пошук_за_css"]:
                new_action["selector"] = extra_entry.get()
            if delay_value:
    ...
    def start_bot(self):
        ...
        def run_profile(profile_name, template_name, callback):
            ...
            def execute_action(action, action_callback, retries=2, index=0):
                ...
                try:
                    ...
                    if action['type'] == "подвійний_клік":
                        pyautogui.doubleClick()
                        async_delay(self.root, int(delay * 1000), action_callback)
                    elif action['type'] == "клік":
                        pyautogui.click()
                        async_delay(self.root, int(delay * 1000), action_callback)
                    elif action['type'] == "введення_тексту":
                        logger.info(f"Введення тексту: {action['text']} на координатах ({x}, {y})")
                        pyautogui.click(x, y)
                        time.sleep(1)
                        pyautogui.click(x, y)
                        time.sleep(0.5)
                        keyboard.write(action['text'], delay=0.1)
                        time.sleep(0.5)
                        ...
                        if not self.page:
                            raise ValueError("Playwright не ініціалізований")
                        selector = action.get("selector", "")
                        if not selector:
                            logger.info(f"Селектор для {action['type']} порожній, пропускаємо")
                            async_delay(self.root, int(delay * 1000), action_callback)
                            return
                        try:
                            if action['type'] == "пошук_за_xpath":
                                locator = self.page.locator(f"xpath={selector}")
                            else:  # пошук_за_css
                                locator = self.page.locator(selector)
                            locator.clear()
                            locator.fill(action.get("text", ""))
                            logger.info(f"Виконано {action['type']} з селектором: {selector}")
                            async_delay(self.root, int(delay * 1000), action_callback)
                        except Exception as e:
                            log_error(f"Помилка виконання {action['type']} з селектором {selector}", e)
                            if retries > 0:
            ...
            def process_actions(index=0):
                if not self.running or index >= len(self.action_manager.data["templates"][template_name]["actions"]):
                    logger.info(f"Перевірка завершення профілю: {profile_name}, шаблон: {template_name}, індекс: {index}")
        ...
        def run_template(template_index=0):
            ...
            selected_profiles = self.action_manager.data["templates"][template_name]["profiles"]
